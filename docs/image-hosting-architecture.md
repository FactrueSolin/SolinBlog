...
图床页面的核心体验不是“文件管理器”，而是“为 AI 发布文章快速得到可用图片 URL”。用户进入页面后应在 30 秒内完成：输入 token、上传图片、复制公开 URL 或 Markdown 图片语法。管理能力服务于纠错和复用，不应压过上传主流程。

第一版 UX 原则：

| 原则 | 设计含义 |
| --- | --- |
| 上传优先 | 页面首屏必须直接暴露上传区，不能先要求进入复杂列表页 |
| 复制优先 | 上传成功后默认高亮公开 URL，并同时提供 Markdown、HTML 两种复制格式 |
| 状态明确 | token、上传、复制、删除、替换都必须有可见状态，不只依赖浏览器 alert |
| 不做导航入口 | `/image` 是知道地址的人使用的管理页，不进入首页和站点 header |
| 低记忆负担 | 用户不需要记住图片 ID；列表以缩略图、时间、尺寸和描述文本辅助识别 |

### 页面信息架构

页面采用单页管理台结构，桌面端为左右分栏，移动端为纵向堆叠：
...
| 详情抽屉 | 右侧浮层或列表内展开 | 底部抽屉 | 查看完整元数据，编辑 alt、description |

桌面端建议使用 `minmax(320px, 420px) 1fr` 的两列布局：左侧稳定承载上传任务，右侧承载列表。移动端在 `720px` 以下改为单列，上传区在前，管理区在后，避免小屏上同时出现两个主要任务。

### 核心用户流程

上传流程固定为：进入 `/image` -> 输入 token -> 拖拽或选择图片 -> 可选填写 alt 和 description -> 点击上传 -> 成功后复制 URL。上传成功后，焦点和视觉高亮应落在“复制 Markdown”按钮上，因为博客写作场景最常用的是 Markdown 图片语法。

管理流程固定为：输入 token -> 加载列表 -> 搜索或滚动定位图片 -> 执行复制、编辑、替换或删除。列表默认按 `created_at desc` 展示，最新上传的图片必须立即插入列表顶部，不要求用户手动刷新。

替换流程必须明确提示：`image_id` 保持不变，但公开 URL 可能因为扩展名变化而改变。替换成功后应显示新旧 URL 对比，并提示旧 filename URL 将返回 404。

删除流程使用二次确认，不使用浏览器原生 `confirm`。确认文案应包含图片 ID 或文件名，例如：`确认删除 img_a1b2c3d4？删除后公开 URL 会立即失效。` 删除成功后从列表移除，并在页面底部或列表顶部显示可关闭的结果提示。
...
| 文件类型不支持 | 在上传卡片内展示错误，不清空已填写的 alt 和 description |
| 上传中 | 显示文件名、大小和进度感知状态；第一版没有真实进度也要显示 loading 状态 |
| 上传成功 | 显示预览图、尺寸、大小、公开 URL，并提供一键复制 URL、Markdown、HTML |
| 复制成功 | 对应按钮短暂变为“已复制”，不要弹窗打断流程 |
| 列表为空 | 展示空状态：“还没有图片，先上传一张用于文章发布” |
| 列表加载失败 | 保留上传区可用，列表区单独展示错误和重试按钮 |
| 删除成功 | 从列表移除图片，显示非阻塞 toast 或 inline notice |
| 替换成功 | 当前列表项更新预览和元数据，并重新提供复制入口 |

### 上传区设计

上传卡片必须支持点击选择和拖拽上传。卡片内展示支持格式和大小上限，例如：`PNG / JPEG / WEBP / GIF / BMP，最大 10MB`，大小上限从后端错误或前端配置文案中保持一致。

选择文件后，上传按钮文案从“选择图片”切换为“上传并生成 URL”。如果浏览器能本地预览，应在上传前展示缩略图、文件名、大小；本地预览失败不阻止上传，以服务端校验为准。

`alt` 输入框面向公开语义，placeholder 建议为：`图片替代文本，用于文章可访问性`。`description` 面向管理备注，placeholder 建议为：`内部备注，例如文章标题、用途或来源`。
...
| 格式 | 示例 | 默认优先级 |
| --- | --- | --- |
| URL | `https://example.com/images/img_xxx/img_xxx.png` | 第二 |
| Markdown | `![alt](https://example.com/images/img_xxx/img_xxx.png)` | 第一 |
| HTML | `<img src="https://example.com/images/img_xxx/img_xxx.png" alt="alt">` | 第三 |

结果区不自动跳转到公开 URL，避免用户离开管理页。可以提供“打开图片”链接，使用新标签页打开。

### 管理列表设计

列表项以图片缩略图为主识别信息，每项展示：缩略图、alt 或 filename、尺寸、文件大小、创建时间、复制按钮、更多操作。没有 alt 时，用 filename 作为主标题；description 只展示一行摘要，完整内容放入详情抽屉。

列表顶部提供搜索框，搜索 placeholder 为：`搜索 ID、文件名、alt、备注`。搜索提交后调用 `GET /api/images?q=...`，不要在前端只过滤当前页，避免用户误以为历史图片不存在。

分页第一版使用“加载更多”按钮而不是页码。按钮文案根据返回数量变化：还有更多时显示“加载更多”，没有更多时显示“已显示全部图片”。这比页码更适合时间倒序的资产流。

### 编辑、替换与删除

编辑元数据使用详情抽屉或列表内展开表单，只允许修改 alt 和 description。保存成功后直接更新当前列表项，不整页刷新。

替换图片入口放在“更多操作”中，避免和普通上传混淆。替换前展示当前图片缩略图和即将上传的新文件信息；替换成功后，结果区复用上传成功的复制组件，提醒用户复制新 URL。

删除入口必须在视觉上弱于复制和编辑，且进入二次确认。删除失败如果返回 `404 image_not_found`，前端应提示“图片已不存在”，并从列表移除该项，避免用户被过期状态卡住。

### 文案与可访问性

所有按钮文案使用动词开头：`上传图片`、`复制 Markdown`、`保存描述`、`替换图片`、`删除图片`。错误信息直接说明用户能做什么，例如：`图片超过 10MB，请压缩后重新上传`，不要只展示 `payload_too_large`。

缩略图必须使用用户填写的 `alt`；如果 alt 为空，管理页缩略图可使用空 alt 并通过相邻文本表达图片身份，避免把文件名当成无意义的朗读内容。所有复制按钮都应有明确的 `aria-label`，例如：`复制 img_a1b2c3d4 的 Markdown 图片语法`。

### 前端状态模型

页面状态建议保持简单，不引入前端框架也能实现：

| 状态 | 内容 |
| --- | --- |
| `token` | 来自输入框和 `sessionStorage`，只在浏览器会话内保存 |
| `selectedFile` | 当前待上传文件和本地预览 URL |
| `uploadState` | `idle`、`validating`、`uploading`、`success`、`error` |
| `lastUpload` | 最近一次上传或替换成功返回的 `ImageMeta` 和复制格式 |
| `images` | 当前已加载图片列表 |
| `listState` | `idle`、`loading`、`ready`、`error` |
| `editingImageId` | 当前正在编辑或展开详情的图片 ID |

Token 可以写入 `sessionStorage`，不建议写入 `localStorage`。页面刷新后保留当前会话的 token 即可，关闭浏览器后自然失效，符合这个项目“轻管理台”的边界。
